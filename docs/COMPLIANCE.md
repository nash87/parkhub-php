# Legal Compliance Matrix — ParkHub PHP
...
| Art. 17 — Right to erasure | `POST /api/v1/users/me/anonymize` (anonymization) + `DELETE /api/v1/users/me/delete` (hard delete) | GDPR module |
...
|------------------|---------------------|------------------|
| Risk management | Security headers, rate limiting, input validation | SecurityHeaders middleware |
| Incident handling | Audit log, breach notification template | Audit Log |
| Business continuity | Docker + backup support, database portability (MySQL/PostgreSQL/SQLite) | Deployment |
| Supply chain security | All dependencies MIT/Apache-2.0/BSD; no proprietary dependencies | [LICENSE-THIRD-PARTY.md](/LICENSE-THIRD-PARTY.md) |
| Encryption | bcrypt passwords, TLS in transit, optional encryption at rest | Core |
...
|----------|-------------|-------------|-----------|------------|
| **User data** | name, email, username, phone, picture, department, role | Art. 6 lit. b | Until deletion | bcrypt (password), TLS (transit) |
...
*This compliance matrix covers ParkHub PHP v3.2.0. It does not constitute legal advice.
