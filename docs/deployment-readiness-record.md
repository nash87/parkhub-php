...
Use this record before a ParkHub PHP deployment is opened for production,
...
| Launch approver | |
| Target launch date | |
| ParkHub PHP commit / tag | |
| CI run / local fop attestation | |
| Nido/fop legal catalog (`nido legal catalog --json` / `fop legal catalog`) generated_at / source_revision | |
| Review record owner | |
