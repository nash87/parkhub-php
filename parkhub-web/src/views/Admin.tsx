...
export function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[28px] border border-surface-200 bg-gradient-to-br from-surface-900 via-surface-800 to-surface-950 px-6 py-6 text-white dark:border-surface-700">
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/75">
              <span className="inline-block h-2 w-2 rounded-full bg-emerald-400" />
              Admin control plane
            </div>
            <h1 className="text-3xl font-semibold tracking-[-0.04em]">{t('admin.title')}</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">{t('admin.subtitle')}</p>
            <div className="mt-5 flex flex-wrap gap-3">
              <AdminHeroStat label={t('admin.users')} value="24" />
              <AdminHeroStat label={t('admin.lots')} value="6" />
              <AdminHeroStat label={t('admin.announcements')} value="3 live" />
              <AdminHeroStat label={t('admin.rateLimits', 'Rate Limits')} value="stable" />
            </div>
          </div>
          <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45">Operational focus</p>
...
function AdminHeroStat({ label, value }: { label: string; value: string }) {
  return (
