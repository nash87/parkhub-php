...
/** Get the next available start number based on all existing rows */
function getNextStartNumber(rows: RowEditorConfig[]): number {
  if (rows.length === 0) return 1;
  let maxEnd = 0;
  for (const row of rows) {
    const end = row.startNumber + row.slotCount - 1;
    if (end > maxEnd) maxEnd = end;
  }
  return maxEnd + 1;
}

/** Get auto-label letter for a row index (A, B, C, ...) */
...
/** Create smart default rows for a new lot */
function createDefaultRows(): RowEditorConfig[] {
  return [
    {
      id: generateId(),
      side: 'top',
...
export function LotLayoutEditor({ initialLayout, lotName: initName, onSave, onCancel }: LotLayoutEditorProps) {
  ...
  const initRows: RowEditorConfig[] = initialLayout?.rows.map((r) => ({
    ...
    slots: r.slots,
  })) || (initialLayout ? [] : createDefaultRows());
  ...
  const updateSlotNumber = useCallback((rowIdx: number, slotIdx: number, value: string) => {
    setRows((prev) => {
      const next = [...prev];
      const row = { ...next[rowIdx] };
      const slots = [...row.slots];
      slots[slotIdx] = { ...slots[slotIdx], number: value };
      row.slots = slots;
      next[rowIdx] = row;
      return next;
    });
  ...
  const updateSlotStatus = useCallback((rowIdx: number, slotIdx: number, status: SlotConfig['status']) => {
    setRows((prev) => {
      const next = [...prev];
      const row = { ...next[rowIdx] };
      const slots = [...row.slots];
      slots[slotIdx] = { ...slots[slotIdx], status };
      row.slots = slots;
      next[rowIdx] = row;
      return next;
    });
  ...
  const addRow = (side: 'top' | 'bottom') => {
    setRows((prev) => {
      const nextStart = getNextStartNumber(prev);
      const newLabel = autoLabel(prev.length);
      return [
        ...prev,
        {
          id: generateId(),
  ...
  const duplicateRow = (idx: number) => {
    setRows((prev) => {
      ...
      const dup: RowEditorConfig = {
        ...src,
        id: generateId(),
        label: src.label + t('lotEditor.copy', ' (Copy)'),
        slotCount: src.slotCount,
        startNumber: nextStart,
        slots: buildSlots(src.slotCount, nextStart),
      };
      const next = [...prev];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('lotEditor.lotName', 'Parking Lot Name')}
          </label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t('lotEditor.namePlaceholder', 'e.g. Company Parking')}
            className="input"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('lotEditor.drivewayLabel', 'Driveway Label')}
          </label>
          <input
            type="text"
            value={roadLabel}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{t('lotEditor.rows', 'Rows')}</h3>
          <div className="flex gap-2">
            <button onClick={() => addRow('top')} className="btn btn-secondary btn-sm">
              <Plus weight="bold" className="w-3.5 h-3.5" />
              {t('lotEditor.addTopRow', 'Top Row')}
            </button>
            <button onClick={() => addRow('bottom')} className="btn btn-secondary btn-sm">
              <Plus weight="bold" className="w-3.5 h-3.5" />
              {t('lotEditor.addBottomRow', 'Bottom Row')}
            </button>
          </div>
        </div>
              ...
              >
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        placeholder={t('lotEditor.rowPlaceholder', 'Row') + ' ' + autoLabel(idx)}
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      >
                        {isEditing ? <Check weight="bold" className="w-4 h-4" /> : <PencilSimple weight="bold" className="w-4 h-4" />}
                      </button>
                      <button onClick={() => moveRow(idx, -1)} className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500" title={t('lotEditor.moveUp', 'Move Up')}>
                        <ArrowUp weight="bold" className="w-4 h-4" />
                      </button>
                      <button onClick={() => moveRow(idx, 1)} className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500" title={t('lotEditor.moveDown', 'Move Down')}>
                        <ArrowDown weight="bold" className="w-4 h-4" />
                      </button>
                      <button onClick={() => duplicateRow(idx)} className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500" title={t('lotEditor.duplicate', 'Duplicate')}>
                        <Copy weight="bold" className="w-4 h-4" />
                      </button>
                      <button onClick={() => removeRow(idx)} className="p-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500" title={t('lotEditor.delete', 'Delete')}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="text-xs text-gray-500 dark:text-gray-400">{t('lotEditor.slotCount', 'Slot Count')}</label>
                      <input
                        type="number"
                        min={1}
                        max={50}
                    ...
                    <div>
                      <label className="text-xs text-gray-500 dark:text-gray-400">{t('lotEditor.startNumber', 'Start Number')}</label>
                      <input
                        type="number"
                        min={1}
                    ...
                    <div className="col-span-2 sm:col-span-1 flex items-end">
                      <span className="text-xs text-gray-400 dark:text-gray-500 pb-2">
                        {t('lotEditor.slotRange', 'Start: {{start}}, End: {{end}} ({{count}} slots)', {
                          start: row.startNumber,
                          end: row.startNumber + row.slotCount - 1,
                          count: row.slotCount,
                        })}
                      </span>
                    </div>
                  ...
                  {/* Slot editing - edit mode vs quick toggle mode */}
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">
                      {t('lotEditor.slotsClickStatus', 'Slots')}
                    </label>
                    {isEditing ? (
                        ...
                        {row.slots.map((slot, si) => (
                          <div key={slot.id} className="flex items-center gap-2">
                            <input
                              type="text"
                              value={slot.number}
                              onChange={(e) => updateSlotNumber(idx, si, e.target.value)}
                              className="input !py-1 !px-2 !text-sm w-20"
                            />
                            <select
                              value={slot.status}
                              onChange={(e) => updateSlotStatus(idx, si, e.target.value as SlotConfig['status'])}
                              className="input !py-1 !px-2 !text-sm w-32"
                            >
                              <option value="available">{t('lotEditor.statusAvailable', 'Available')}</option>
                              <option value="disabled">{t('lotEditor.statusDisabled', 'Disabled')}</option>
                              <option value="blocked">{t('lotEditor.statusBlocked', 'Blocked')}</option>
                            </select>
                          </div>
                        ))}
                      </div>
        ...
        {rows.length === 0 && (
          <div className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">
            {t('lotEditor.noRows', 'No rows added yet.')}
          </div>
        )}
      </div>
      ...
      {rows.length > 0 && (
        <div>
          ...
          >
            ...
            {showPreview ? t('lotEditor.hidePreview', 'Hide Preview') : t('lotEditor.showPreview', 'Show Preview')}
          </button>
