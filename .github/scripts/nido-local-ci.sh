...
usage() {
  cat <<'EOF'
Usage: .github/scripts/nido-local-ci.sh [--profile pr|full|cd] [--dry-run] [--post-status] [--background]
...
.nido/reports/local-ci-<profile>-<sha>-bg.log, and returns immediately.
...
Profiles:
  pr    Fast PR gate: composer + Pint + PHPStan + PHPUnit + Vitest +
        ...
        touch their inputs. Set NIDO_LOCAL_CI_NO_DIFF_AWARE=1 to force every
        PR step.
...
Environment overrides (NIDO_LOCAL_CI_* primary; FOP_LOCAL_CI_* are compat fallbacks):
  NIDO_LOCAL_CI_STATUS_REPO     owner/repo for status post (else autodetected
                               from git remotes named github → upstream → origin).
  NIDO_LOCAL_CI_NO_DIFF_AWARE=1 disable diff-aware skipping on pr profile.
  NIDO_LOCAL_CI_DIFF_PATHS      newline-delimited diff path override for
                               contract tests and explicit local reruns.
  NIDO_LOCAL_CI_BG_LOG_DIR      background log directory override.
  NIDO_LOCAL_CI_QUEUE_BIN       queue wrapper binary. Defaults to `nido` when it
                               supports `build`, otherwise `fop`.
  NIDO_LOCAL_CI_DIRECT          1 = bypass the queue wrapper and
...
# ---------------------------------------------------------------------------
# Env-var resolution: NIDO_LOCAL_CI_* primary; FOP_LOCAL_CI_* compat fallback.
# Resolved early so all subsequent code uses only NIDO_LOCAL_CI_* names.
# ---------------------------------------------------------------------------
NIDO_LOCAL_CI_STATUS_REPO="${NIDO_LOCAL_CI_STATUS_REPO:-${FOP_LOCAL_CI_STATUS_REPO:-}}"
NIDO_LOCAL_CI_NO_DIFF_AWARE="${NIDO_LOCAL_CI_NO_DIFF_AWARE:-${FOP_LOCAL_CI_NO_DIFF_AWARE:-}}"
NIDO_LOCAL_CI_DIFF_PATHS="${NIDO_LOCAL_CI_DIFF_PATHS:-${FOP_LOCAL_CI_DIFF_PATHS:-}}"
NIDO_LOCAL_CI_BG_LOG_DIR="${NIDO_LOCAL_CI_BG_LOG_DIR:-${FOP_LOCAL_CI_BG_LOG_DIR:-}}"
NIDO_LOCAL_CI_QUEUE_BIN="${NIDO_LOCAL_CI_QUEUE_BIN:-${FOP_LOCAL_CI_QUEUE_BIN:-}}"
NIDO_LOCAL_CI_DIRECT="${NIDO_LOCAL_CI_DIRECT:-${FOP_LOCAL_CI_DIRECT:-0}}"
NIDO_LOCAL_CI_LARAVEL_PORT="${NIDO_LOCAL_CI_LARAVEL_PORT:-${FOP_LOCAL_CI_LARAVEL_PORT:-}}"
NIDO_LOCAL_CI_RUN_SCHEMATHESIS="${NIDO_LOCAL_CI_RUN_SCHEMATHESIS:-${FOP_LOCAL_CI_RUN_SCHEMATHESIS:-0}}"
NIDO_LOCAL_CI_RUN_INFECTION="${NIDO_LOCAL_CI_RUN_INFECTION:-${FOP_LOCAL_CI_RUN_INFECTION:-0}}"
NIDO_LOCAL_CI_E2E_DB="${NIDO_LOCAL_CI_E2E_DB:-${FOP_LOCAL_CI_E2E_DB:-}}"
export NIDO_LOCAL_CI_STATUS_REPO NIDO_LOCAL_CI_NO_DIFF_AWARE NIDO_LOCAL_CI_DIFF_PATHS
export NIDO_LOCAL_CI_BG_LOG_DIR NIDO_LOCAL_CI_QUEUE_BIN NIDO_LOCAL_CI_DIRECT
export NIDO_LOCAL_CI_LARAVEL_PORT NIDO_LOCAL_CI_RUN_SCHEMATHESIS NIDO_LOCAL_CI_RUN_INFECTION
export NIDO_LOCAL_CI_E2E_DB

if [[ "$background" -eq 1 ]]; then
  repo_root_for_bg="$(git rev-parse --show-toplevel)"
  bg_log_dir="${NIDO_LOCAL_CI_BG_LOG_DIR:-$repo_root_for_bg/.nido/reports}"
  mkdir -p "$bg_log_dir"
  ...
  echo "nido-local-ci backgrounded: profile=$profile log=$bg_log"
...
queue_bin="${NIDO_LOCAL_CI_QUEUE_BIN:-}"
if [[ -z "$queue_bin" ]]; then
...
# Canonical context posted first; compat context posted alongside for the
# transition period. The orchestrator will flip branch protection from
# fop/local-ci/pr to nido/local-ci/pr in a separate step.
context="nido/local-ci/${profile}"
compat_context="fop/local-ci/${profile}"   # retire after branch-protection flip

# Canonical report path (.nido/); compat copy written to (.fop/) for
# older scripts and the check-local-ci-report.sh fop fallback path.
report_dir="$repo_root/.nido/reports"
compat_report_dir="$repo_root/.fop/reports"
report_path="$report_dir/local-ci-${profile}-${sha}.json"
compat_report_path="$compat_report_dir/local-ci-${profile}-${sha}.json"
...
compute_design_smoke_gate() {
  if [[ "${NIDO_LOCAL_CI_NO_DIFF_AWARE:-}" == "1" ]] || [[ "$profile" != "pr" ]]; then
  ...
  local base_label=""
  if [[ -n "${NIDO_LOCAL_CI_DIFF_PATHS:-}" ]]; then
    design_diff_paths="${NIDO_LOCAL_CI_DIFF_PATHS}"
    base_label="override"
  fi
...
# sibling nido-local-ci run for a different PR worktree on the same desktop.
...
#
# Order of preference:
#   1. NIDO_LOCAL_CI_LARAVEL_PORT (explicit operator override)
...
allocate_laravel_port() {
  if [[ -n "${NIDO_LOCAL_CI_LARAVEL_PORT:-}" ]]; then
    printf '%s' "${NIDO_LOCAL_CI_LARAVEL_PORT}"
    return 0
  fi
...
compute_diff_paths() {
  if [[ "$profile" != "pr" || "${NIDO_LOCAL_CI_NO_DIFF_AWARE:-}" == "1" ]]; then
    enable_all_pr_steps
    return 0
  fi
  ...
  if [[ -n "${NIDO_LOCAL_CI_DIFF_PATHS:-}" ]]; then
    diff_paths="${NIDO_LOCAL_CI_DIFF_PATHS}"
  else
    local base_ref=""
    for candidate in github/main upstream/main origin/main main; do
      if git rev-parse --verify --quiet "$candidate" >/dev/null; then
...
status_repo() {
  if [[ -n "${NIDO_LOCAL_CI_STATUS_REPO:-}" ]]; then
    printf '%s\n' "$NIDO_LOCAL_CI_STATUS_REPO"
    return 0
  fi
  ...
  echo "unable to derive GitHub owner/repo; set NIDO_LOCAL_CI_STATUS_REPO" >&2
  return 1
}

# Post a single commit status context via gh api.
_post_one_status() {
  local ctx="$1"
  local state="$2"
  local description="$3"
  ...
  if ! out="$(gh api \
    ...
    if echo "$out" | grep -qE "No commit found for SHA|HTTP 422"; then
      echo "Skipping status post for ${ctx} — commit ${sha:0:8} not yet on GitHub (will land after push; gate falls back to timeout)." >&2
      return 0
    fi
    echo "$out" >&2
    return 1
...
post_commit_status() {
  ...
  # Post canonical context first.
  _post_one_status "$context" "$state" "$description"
  # Post compat context alongside (retire after branch-protection flip to nido/local-ci/pr).
  _post_one_status "$compat_context" "$state" "$description"
}
...
write_report() {
  ...
  local diff_enabled_str diff_php_str diff_frontend_str diff_workflows_str
  local diff_e2e_str diff_image_str diff_nix_dev_str diff_security_str
  [[ "$diff_aware_enabled" -eq 1 ]] && diff_enabled_str=true   || diff_enabled_str=false
  [[ "$diff_touch_php" -eq 1 ]]     && diff_php_str=true       || diff_php_str=false
  [[ "$diff_touch_frontend" -eq 1 ]] && diff_frontend_str=true || diff_frontend_str=false
  [[ "$diff_touch_workflows" -eq 1 ]] && diff_workflows_str=true || diff_workflows_str=false
  [[ "$diff_touch_e2e" -eq 1 ]]     && diff_e2e_str=true       || diff_e2e_str=false
  [[ "$diff_touch_image" -eq 1 ]]   && diff_image_str=true     || diff_image_str=false
  [[ "$diff_touch_nix_dev" -eq 1 ]] && diff_nix_dev_str=true   || diff_nix_dev_str=false
  [[ "$diff_touch_security" -eq 1 ]] && diff_security_str=true || diff_security_str=false
  ...
  \"context\": \"$context\",
  \"diff_aware\": {
    \"enabled\": $diff_enabled_str,
    \"php\": $diff_php_str,
    \"frontend\": $diff_frontend_str,
    \"workflows\": $diff_workflows_str,
    \"e2e\": $diff_e2e_str,
    \"image\": $diff_image_str,
    \"nix_dev\": $diff_nix_dev_str,
    \"security\": $diff_security_str
  }
}"
  # Write canonical report to .nido/reports/.
  mkdir -p "$report_dir"
  printf '%s\n' "$report_json" > "$report_path"
  # Write compat copy to .fop/reports/ for older scripts and the fallback path.
  mkdir -p "$compat_report_dir"
  printf '%s\n' "$report_json" > "$compat_report_path"
}
...
# Setting NIDO_LOCAL_CI_DIRECT=1 bypasses the queue wrapper and
...
run_step() {
  ...
  if [[ "${NIDO_LOCAL_CI_DIRECT:-0}" == "1" ]] || ! command -v "$queue_bin" >/dev/null 2>&1; then
...
# would be pure overhead inside the nido queue.
run_direct() {
  local name="$1"
  local command="$2"
  printf '\n==> %s\n' "$name"
...
mark_failure() {
  ...
  post_commit_status "failure" "nido local ${profile} failed" || true
}
...
export SERVER_PORT
printf 'ℹ Laravel dev-server port: %s (override with NIDO_LOCAL_CI_LARAVEL_PORT)\n' "$SERVER_PORT"

post_commit_status "pending" "nido local ${profile} running"
...
if (( diff_touch_frontend || diff_touch_e2e )); then
  ...
  # hard frontend gates and make the nido wrapper advisory too, so host pressure
...
if [[ "$profile" == "full" || "$profile" == "cd" ]]; then
  ...
  #   2. caller opted in via NIDO_LOCAL_CI_RUN_SCHEMATHESIS=1?
  ...
  run_step "schemathesis contract fuzz (soft, opt-in)" "if [[ \"\${NIDO_LOCAL_CI_RUN_SCHEMATHESIS:-0}\" != \"1\" ]]; then echo 'schemathesis disabled by default; export NIDO_LOCAL_CI_RUN_SCHEMATHESIS=1 with a running php artisan serve on :8082 to enable'; elif command -v schemathesis >/dev/null 2>&1; then ./scripts/dump-openapi.sh && schemathesis run --checks=all --hypothesis-max-examples=50 docs/openapi/php.json --base-url=http://127.0.0.1:8082 || echo 'schemathesis returned non-zero (soft on full profile)'; else echo 'schemathesis not installed; skipping'; fi"
...
if [[ "$trivy_should_run" -eq 0 ]]; then
  ...
  post_commit_status "failure" "nido local ${profile} failed: trivy not installed"
  exit 1
else
...
# scan twice per nido-local-ci invocation — addresses #409 review.
...
if [[ "$dry_run" -eq 1 ]]; then
  ...
  post_commit_status "success" "nido local ${profile} passed"
